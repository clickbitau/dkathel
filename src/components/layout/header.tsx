...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/50 dark:border-slate-700/50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md shadow-sm">
            ...
            <div className="flex">
              {localizedNavigation.filter(item =>
                ...
                <Link
                  key={item.href}
                  href={item.href}
                  className={`px-4 py-2.5 text-[1.2rem] leading-6 font-medium text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 relative group ${index > 0 ? 'ml-2' : ''}`}
                >
                  {typeof item.name === 'object' ? item.name[locale as keyof typeof item.name] : item.name}
                  <span className="absolute left-4 right-4 -bottom-0.5 h-0.5 bg-gradient-to-r from-blue-500 to-purple-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
                </Link>
              ))}
            </div>
          </div>
          ...
          <div className="hidden lg:flex lg:flex-1 lg:items-center lg:justify-start">
            <div className="flex items-center">
              {NAVIGATION.filter(item =>
                ['/entrepreneur', '/certifications'].includes(item.href)
              ).map((item, index) => (
                <Link
                  key={item.href}
                  href={`/${locale}${item.href}`}
              ...
              <div className="flex items-center space-x-4 ml-6 pl-6 border-l border-slate-200 dark:border-slate-700 h-10">
                <LanguageSwitcher />
                <ThemeToggle />
                <Link href={`/${locale}/contact`}>
                  <Button
                    variant="outline"
                    className="bg-gradient-to-r from-blue-500 to-purple-600 text-white border-0 hover:from-blue-600 hover:to-purple-700 shadow-lg hover:shadow-xl transition-all duration-300 whitespace-nowrap px-6 py-2.5 text-[1.1rem]"
                  >
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white/95 dark:bg-slate-900/95 backdrop-blur-md px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-slate-200/20 dark:ring-slate-700/20">
            <div className="flex items-center justify-between">
              <Link href={locale === 'en' ? '/' : `/${locale}`} className="-m-1.5 p-1.5 flex items-center gap-3">
                <div className="w-8 h-8 flex items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 p-0.5">
                  <Image
                    src="/images/dk-profile.png"
                ...
                <span className={`text-lg font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent ${locale === 'bn' ? 'font-heading-bn' : 'font-heading-en'}`}>
                  DKathel
                </span>
              </Link>
              <Button
            ...
            <div className="mt-6 flow-root">
              <div className="-my-6 divide-y divide-slate-200/20 dark:divide-slate-700/20">
                ...
                <div className="py-6">
                  <Link href={`/${locale}/contact`}>
                    <Button className="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white border-0 hover:from-blue-600 hover:to-purple-700">
                      {locale === 'en' ? 'Contact Me' : 'যোগাযোগ করুন'}
                    </Button>
                  </Link>
                </div>
