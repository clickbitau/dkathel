          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {Object.entries(skills).map(([category, categorySkills]: [string, any], index) => (
              <div
                ...
                className="group"
              >
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 border border-gray-200 dark:border-slate-700 hover:shadow-2xl transition-all duration-500 transform hover:scale-105">
                  ...
                  <div className="space-y-4">
                    {categorySkills.map((skill: any) => (
                      <div key={skill.id} className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-slate-700 dark:to-slate-600 rounded-xl p-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className={`font-semibold text-gray-900 dark:text-white ${locale === 'bn' ? 'font-body-bn' : 'font-body-en'}`}>{skill.name}</span>
                          <span className={`text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/30 px-3 py-1 rounded-full capitalize ${locale === 'bn' ? 'font-body-bn' : 'font-body-en'}`}>
                            {skill.proficiency}
                          </span>
                        </div>
                        {skill.description && (
